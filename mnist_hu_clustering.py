...
for i in range(len(x_train)):
    ...
    h = cv2.HuMoments(cv2.moments(img)).flatten()
    #h = hog.compute(img)
...
for i in range(0,10):
    for j in range(0,10):
        sim = sqrt(mean_squared_error(mean_features[i],mean_features[j]))
        diff[i,j] = sim
